...
CompassHeading mag_heading = 0;

//sets filter threshold for callback to 360/90 = 4 degrees
static const CompassHeading threshold_value = TRIG_MAX_ANGLE/90;
...
static void bg_update_proc(Layer *layer, GContext *ctx) {
  ...
  for (int i = 0; i < NUM_CLOCK_TICKS; ++i) {
    gpath_rotate_to(tick_paths[i], TRIG_MAX_ANGLE * mag_heading / 360);
...
static void hands_update_proc(Layer *layer, GContext *ctx) {
  ...
  int32_t second_angle = TRIG_MAX_ANGLE * t->tm_sec / 60 + TRIG_MAX_ANGLE * mag_heading / 360;
  ...
  gpath_rotate_to(minute_arrow, TRIG_MAX_ANGLE * t->tm_min / 60 + TRIG_MAX_ANGLE * mag_heading / 360);
  gpath_draw_filled(ctx, minute_arrow);
  gpath_draw_outline(ctx, minute_arrow);

  gpath_rotate_to(hour_arrow, (TRIG_MAX_ANGLE * (((t->tm_hour % 12) * 6) + (t->tm_min / 10))) / (12 * 6) + TRIG_MAX_ANGLE * mag_heading / 360);
...
//will be triggered every time a degree change of 4+ occurs
static void compass_heading_handler(CompassHeadingData heading_data){
  // if (heading_data.is_declination_valid){
    mag_heading = TRIGANGLE_TO_DEG(heading_data.magnetic_heading);
  // } else {
  //   mag_heading = 0;
  // }
  layer_mark_dirty(window_get_root_layer(window));
...
static void init(void) {
  //initialize compass
  compass_service_set_heading_filter(threshold_value);
  ...
  for (int i = 0; i < NUM_CLOCK_TICKS; ++i) {
    ...
    gpath_move_to(tick_paths[i], center);
  }
